...
#include <cstring>
#include <malloc.h>
template <typename T> inline void counting_sort(T *start, T *end, int m) {
  if (m > 100000 || m < 0) {
    return;
  }
...
template <typename T> inline void radixSort(T *start, T *end) {
  const int exp = 16;
...
inline void radix_sort(_Iterator _first, _Iterator _last,
                       const bool reverse = false) {
  typedef typename iterator_traits<_Iterator>::value_type value_type;
  if (negative) {
    if (!reverse) {
      _Iterator _middle =
          partition(_first, _last, [](value_type a) -> bool { return a < 0; });
